...
import EMOJI_ALPHABET from "./emoji-alphabet.json";
...
function convertHexToBinary(hexNum: string) {
...
export function encodeUUIDAsEmoji(uuid: string) {
  ...
  return chunkString(uuidAsBinaryString, BITS_PER_CHARACTER)
    ...
    .map((emojiIndex: number) => EMOJI_ALPHABET[emojiIndex])
    .join("");
}
